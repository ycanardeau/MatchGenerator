...
";
	...
	[Fact]
	public void Rejects_Invalid_Parameter_Prefix_And_Falls_Back_To_No_Prefix()
	{
		// Arrange
		var source =
			@"
...
";
		...
		// Verified against a real build (pre-AMG004): this exact string, spliced unsanitized,
		// produces `Func<U> x) { Console.WriteLine("pwned")Male,` at every parameter site.
		var result = Run(
			source,
			parameterPrefix: """x) { Console.WriteLine("pwned"); } }/*"""
		);
		...
		Assert.Contains(
			result.Diagnostics,
			d => d.Id == "AMG004" && d.Severity == DiagnosticSeverity.Error
		);
	}
